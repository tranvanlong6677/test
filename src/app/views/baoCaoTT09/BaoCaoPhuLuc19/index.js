...
const BaoCaoPhuLuc19ControlView = () => {

  const classes = useStyles();
  const dispatch = useDispatch();
  const dataBaoCaoTT09PhuLuc19 = useSelector(
    state => state.reportTT09Slice.dataBaoCaoTT09PhuLuc19
  );
  ...
  useEffect(() => {
    const currentPage_local = JSON.parse(localStorage.getItem('pageCurrent-vpdl'))
    const searchObject_local = JSON.parse(localStorage.getItem('searchObject-vpdl'))
    ...
    if(!!searchObject_local && searchObject_local !== '' && currentPage_local?.page){
      ...
      dispatch(BaoCaoTT09PhuLuc19(searchObject_local));
  ...
  useEffect(() => {
    return () => {
      localStorage.removeItem('searchObject-vpdl');
      localStorage.removeItem('pageCurrent-vpdl');
    }
  }, []);

  useEffect(() => {
    console.log("check");
    if (dataBaoCaoTT09PhuLuc19 && dataBaoCaoTT09PhuLuc19.total > 1 && nowPage.page == 1) {

      let currentSearchObject = { ...searchObject };
      var total = dataBaoCaoTT09PhuLuc19.total;
  ...
  const changeSearchCondition = (name, value) => {
    ...
    localStorage.setItem('searchObject-vpdl',JSON.stringify(currentSearchObject))
  };
  ...
  const searchFunction = () => {
    ...
    localStorage.setItem('pageCurrent-vpdl',JSON.stringify(currentNowpage));
    localStorage.setItem('searchObject-vpdl',JSON.stringify(currentSearchObject))
    ...
    currentSearchObject.lastTime = currentSearchObject.lastTime
      ...
      : null;
    dispatch(BaoCaoTT09PhuLuc19(currentSearchObject));
  };
  ...
  const downloadFile = () => {
    let currentSearchObject = { ...searchObject };
    var urlBase = `${process.env.REACT_APP_REPORT_URL}/api/v1/report/agency/${localStorage.getItem('agency-id')}/tt09/phuluc19/excel?`;
  ...
  return (
    <Box className={classes.contentContainer}>
      ...
      <div className={classes.dataTable}>
          ...
          >
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell rowSpan={2} align="left">Đơn vị vận tải</StyledTableCell>
                <StyledTableCell rowSpan={2} align="left">Đơn vị truyền dữ liệu</StyledTableCell>
                <StyledTableCell rowSpan={2} align="left">Số ngày không truyền</StyledTableCell>
              </TableRow>
              <TableRow>
              </TableRow>
            </TableHead>
            <TableBody>
              {dataBaoCaoTT09PhuLuc19 &&
                dataBaoCaoTT09PhuLuc19.data &&
                dataBaoCaoTT09PhuLuc19.data.length > 0 &&
                dataBaoCaoTT09PhuLuc19.data.map((row, index) => {
                  return (
                    <StyledTableRow
                      hover
                    ...
                    >
                      ...
                      <StyledTableCell>{""}</StyledTableCell>
                      <StyledTableCell>{row.offlines}</StyledTableCell>
                    </StyledTableRow>
                  );
                })}
            </TableBody>
        ...
        {!dataBaoCaoTT09PhuLuc19 || !dataBaoCaoTT09PhuLuc19.data || dataBaoCaoTT09PhuLuc19.data.length < 1 ?
...
export default BaoCaoPhuLuc19ControlView;
