...
const BaoCaoPhuLuc8ControlView = () => {

  const classes = useStyles();
  const dispatch = useDispatch();
  const dataBaoCaoTT09PhuLuc8 = useSelector(
    state => state.reportTT09Slice.dataBaoCaoTT09PhuLuc8
  );
  ...
  useEffect(() => {
    const currentPage_local = JSON.parse(localStorage.getItem('pageCurrent-pvtd'))
    const searchObject_local = JSON.parse(localStorage.getItem('searchObject-pvtd'))
    ...
    if(!!searchObject_local && searchObject_local !== '' && currentPage_local?.page){
      ...
      dispatch(BaoCaoTT09PhuLuc8(searchObject_local));
  ...
  useEffect(() => {
    return () => {
      localStorage.removeItem('searchObject-pvtd');
      localStorage.removeItem('pageCurrent-pvtd');
    }
  }, []);

  useEffect(() => {
    console.log("check");
    if (dataBaoCaoTT09PhuLuc8 && dataBaoCaoTT09PhuLuc8.total > 1 && nowPage.page == 1) {

      let currentSearchObject = { ...searchObject };
      var total = dataBaoCaoTT09PhuLuc8.total;
  ...
  const changeSearchCondition = (name, value) => {
    ...
    localStorage.setItem('searchObject-pvtd',JSON.stringify(currentSearchObject))
  };
  ...
  const searchFunction = () => {
    ...
    localStorage.setItem('pageCurrent-pvtd',JSON.stringify(currentNowpage));
    localStorage.setItem('searchObject-pvtd',JSON.stringify(currentSearchObject))
    ...
    currentSearchObject.lastTime = currentSearchObject.lastTime
      ...
      : null;
    dispatch(BaoCaoTT09PhuLuc8(currentSearchObject));
  };
  ...
  const downloadFile = () => {
    let currentSearchObject = { ...searchObject };
    var urlBase = `${process.env.REACT_APP_REPORT_URL}/api/v1/report/agency/${localStorage.getItem('agency-id')}/tt09/phuluc8/excel?`;
      ...
      <div className={classes.dataTable}>
        <TableContainer>
          ...
          >
            ...
            <TableBody>
              {dataBaoCaoTT09PhuLuc8 &&
                dataBaoCaoTT09PhuLuc8.data &&
                dataBaoCaoTT09PhuLuc8.data.length > 0 &&
                dataBaoCaoTT09PhuLuc8.data.map((row, index) => {
                  return (
                    <StyledTableRow
                      hover
                    ...
                    >
                      ...
                      <StyledTableCell>{row.distanceViolation}</StyledTableCell>
                      <StyledTableCell>{row.totalDistance}</StyledTableCell>
                      <StyledTableCell>{row.ratioDistanceViolation}</StyledTableCell>
                      <StyledTableCell>{row.timeViolation}</StyledTableCell>
        ...
        {!dataBaoCaoTT09PhuLuc8 || !dataBaoCaoTT09PhuLuc8.data || dataBaoCaoTT09PhuLuc8.data.length < 1 ?
...
export default BaoCaoPhuLuc8ControlView;
