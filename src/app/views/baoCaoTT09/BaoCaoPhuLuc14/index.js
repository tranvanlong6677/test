...
const BaoCaoPhuLuc14ControlView = () => {

  const classes = useStyles();
  const dispatch = useDispatch();
  const dataBaoCaoTT09PhuLuc14 = useSelector(
    state => state.reportTT09Slice.dataBaoCaoTT09PhuLuc14
  );
  ...
  useEffect(() => {
    const currentPage_local = JSON.parse(localStorage.getItem('pageCurrent-ctvp'))
    const searchObject_local = JSON.parse(localStorage.getItem('searchObject-ctvp'))
    ...
    if(!!searchObject_local && searchObject_local !== '' && currentPage_local?.page){
      ...
      dispatch(BaoCaoTT09PhuLuc14(searchObject_local));
  ...
  useEffect(() => {
    return () => {
      localStorage.removeItem('searchObject-ctvp');
      localStorage.removeItem('pageCurrent-ctvp');
    }
  }, []);
  ...
  useEffect(() => {
    console.log("check");
    if (dataBaoCaoTT09PhuLuc14 && dataBaoCaoTT09PhuLuc14.total > 1 && nowPage.page == 1) {

      let currentSearchObject = { ...searchObject };
      var total = dataBaoCaoTT09PhuLuc14.total;
  ...
  const changeSearchCondition = (name, value) => {
    ...
    localStorage.setItem('searchObject-ctvp',JSON.stringify(currentSearchObject))
  };
  ...
  const searchFunction = () => {
    ...
    localStorage.setItem('pageCurrent-ctvp',JSON.stringify(currentNowpage));
    localStorage.setItem('searchObject-ctvp',JSON.stringify(currentSearchObject))
    ...
    currentSearchObject.lastTime = currentSearchObject.lastTime
      ...
      : null;
    dispatch(BaoCaoTT09PhuLuc14(currentSearchObject));
  };
  ...
  const downloadFile = () => {
    let currentSearchObject = { ...searchObject };
    var urlBase = `${process.env.REACT_APP_REPORT_URL}/api/v1/report/agency/${localStorage.getItem('agency-id')}/tt09/phuluc14/excel?`;
  ...
  return (
    <Box className={classes.contentContainer}>
      <div className={classes.titleHeader}>BÁO CÁO CHI TIẾT VI PHẠM TỐC ĐỘ XE CHẠY</div>
      ...
      <div className={classes.dataTable}>
        <TableContainer>
          ...
          >
            ...
            <TableBody>
              {dataBaoCaoTT09PhuLuc14 &&
                dataBaoCaoTT09PhuLuc14.data &&
                dataBaoCaoTT09PhuLuc14.data.length > 0 &&
                dataBaoCaoTT09PhuLuc14.data.map((row, index) => {
                  return (
                    <StyledTableRow
                      hover
                    ...
                    >
                      ...
                      <StyledTableCell>{row.maxSpeed}</StyledTableCell>
        ...
        {!dataBaoCaoTT09PhuLuc14 || !dataBaoCaoTT09PhuLuc14.data || dataBaoCaoTT09PhuLuc14.data.length < 1 ?
...
export default BaoCaoPhuLuc14ControlView;
