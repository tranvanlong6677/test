...
const BaoCaoPhuLuc16ControlView = () => {

  const classes = useStyles();
  const dispatch = useDispatch();
  const dataBaoCaoTT09PhuLuc16 = useSelector(
    state => state.reportTT09Slice.dataBaoCaoTT09PhuLuc16
  );
  ...
  useEffect(() => {
    const currentPage_local = JSON.parse(localStorage.getItem('pageCurrent-vplt'))
    const searchObject_local = JSON.parse(localStorage.getItem('searchObject-vplt'))
    ...
    if(!!searchObject_local && searchObject_local !== '' && currentPage_local?.page){
      ...
      dispatch(BaoCaoTT09PhuLuc16(searchObject_local));
  ...
  useEffect(() => {
    return () => {
      localStorage.removeItem('searchObject-vplt');
      localStorage.removeItem('pageCurrent-vplt');
    }
  }, []);
  ...
  useEffect(() => {
    console.log("check");
    if (dataBaoCaoTT09PhuLuc16 && dataBaoCaoTT09PhuLuc16.total > 1 && nowPage.page == 1) {

      let currentSearchObject = { ...searchObject };
      var total = dataBaoCaoTT09PhuLuc16.total;
  ...
  const changeSearchCondition = (name, value) => {
    ...
    localStorage.setItem('searchObject-vplt',JSON.stringify(currentSearchObject))
  };
  ...
  const searchFunction = () => {
    ...
    localStorage.setItem('pageCurrent-vplt',JSON.stringify(currentNowpage));
    localStorage.setItem('searchObject-vplt',JSON.stringify(currentSearchObject))
    ...
    currentSearchObject.lastTime = currentSearchObject.lastTime
      ...
      : null;
    dispatch(BaoCaoTT09PhuLuc16(currentSearchObject));
  };
  ...
  const downloadFile = () => {
    let currentSearchObject = { ...searchObject };
    var urlBase = `${process.env.REACT_APP_REPORT_URL}/api/v1/report/agency/${localStorage.getItem('agency-id')}/tt09/phuluc16/excel?`;
  ...
  return (
    <Box className={classes.contentContainer}>
      <div className={classes.titleHeader}>BÁO CÁO CHI TIẾT VI PHẠM THỜI GIAN LÁI XE LIÊN TỤC (4 GIỜ)</div>
      ...
      <div className={classes.dataTable}>
        <TableContainer>
          ...
          >
            ...
            <TableBody>
              {dataBaoCaoTT09PhuLuc16 &&
                dataBaoCaoTT09PhuLuc16.data &&
                dataBaoCaoTT09PhuLuc16.data.length > 0 &&
                dataBaoCaoTT09PhuLuc16.data.map((row, index) => {
                  return (
                    <StyledTableRow
                      hover
        ...
        {!dataBaoCaoTT09PhuLuc16 || !dataBaoCaoTT09PhuLuc16.data || dataBaoCaoTT09PhuLuc16.data.length < 1 ?
...
export default BaoCaoPhuLuc16ControlView;
