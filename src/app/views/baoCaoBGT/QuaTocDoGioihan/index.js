...
import { quaTocDoGioiHan,licensePlate,groupID,resetDataSceen } from '../../../../features/reportBgtQC31Slice';
...
useEffect(() => {
  const currentPage_local = JSON.parse(localStorage.getItem('pageCurrent-qtd'))
  const searchObject_local = JSON.parse(localStorage.getItem('searchObject-qtd'))
  ...
  if(!!searchObject_local && searchObject_local !== '' && currentPage_local?.page){
    ...
    dispatch(quaTocDoGioiHan(searchObject_local));
...
useEffect(() => {
  return () => {
    localStorage.removeItem('searchObject-qtd');
    localStorage.removeItem('pageCurrent-qtd');
  }
}, []);
...
useEffect(() => {
  console.log("check");
  if(dataQuaTocDoGioiHan && dataQuaTocDoGioiHan.total >1 && nowPage.page==1){
  ...
  const changeSearchCondition = (name, value) => {
    ...
    localStorage.setItem('searchObject-qtd',JSON.stringify(currentSearchObject))
  };
  ...
  const searchFunction = () => {
    ...
    localStorage.setItem('pageCurrent-qtd',JSON.stringify(currentNowpage));
    localStorage.setItem('searchObject-qtd',JSON.stringify(currentSearchObject))
    console.log("kien")
  ...
  const downloadFile = () => {
    ...
    var urlBase= `${process.env.REACT_APP_REPORT_URL}/api/v1/report/agency/${localStorage.getItem('agency-id')}/overspeed/excel?`;
  ...
  return (
    <Box className={classes.contentContainer}>
          ...
          >
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell align="left">Tốc độ trung bình khi quá tốc độ giới hạn (km/h)</StyledTableCell>
                <StyledTableCell align="left">Tốc độ giới hạn (km/h)</StyledTableCell>
                <StyledTableCell align="left">Tọa độ quá tốc độ giới hạn</StyledTableCell>
                <StyledTableCell align="left">Địa điểm quá tốc độ giới hạn</StyledTableCell>
                ...
                return (
                  ...
                  >
                    ...
                    <StyledTableCell>
                      ...
                      {row.startAt
                        ? moment.unix(row.startAt).format('HH:mm:ss DD/MM/YYYY')
                        : ''}
                    </StyledTableCell>

                    <StyledTableCell>{row.avgSpeed}</StyledTableCell>
                    <StyledTableCell>{row.limitedSpeed}</StyledTableCell>
                    <StyledTableCell>{row.coord[0]} , {row.coord[1]} </StyledTableCell>
      ...
      {!dataQuaTocDoGioiHan || !dataQuaTocDoGioiHan.data || dataQuaTocDoGioiHan.data.length < 1 ?
