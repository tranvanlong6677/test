...
const Toolbar = ({
  ...
  useEffect(() => {
    const queryValue_local = JSON.parse(localStorage.getItem('query-value'));
    if(!!queryValue_local){
      searchRef(queryValue_local);
      setQuery(queryValue_local);
    }
  }, []);
  ...
  const onEnterSearchInput = event => {
    ...
    if (event.keyCode === 13) {
      ...
      setQuery(queryValue);
      searchRef(queryValue);
      localStorage.setItem('query-value',JSON.stringify(queryValue));
    }
  };
