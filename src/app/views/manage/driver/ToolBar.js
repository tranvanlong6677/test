...
const Toolbar = ({
  ...
  useEffect(() => {
    const inputSearch_local = JSON.parse(localStorage.getItem("valueSearch-driver"));
    if(inputSearch_local){
      searchRef(inputSearch_local);
      setQuery(inputSearch_local);
    }
  }, []);
  ...
  useEffect(() => {
    return () => {
      localStorage.removeItem('valueSearch-driver');
    }
  }, []);
  ...
  const onEnterSearchInput = event => {
    ...
    if (event.keyCode === 13) {
      ...
      setQuery(queryValue);
      searchRef(queryValue);
      localStorage.setItem('valueSearch-driver',JSON.stringify(queryValue))
    }
  };
  ...
  return (
    <div className={clsx(classes.root, className)} {...rest}>
      <CreateDriver open={openCreateModal} closeRef={closeModal} />
      <Box>
